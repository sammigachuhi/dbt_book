...
Note the `updated_at` column. Omit this and your snapshot won't be able to track any changes. Run `dbt snapshot`.

Check `timestamp_snapshot` table.
...
Run `dbt snapshot`. Don't omit the `updated_at` column since this is what the `timestamp` strategy relies on.

Now check if dbt has been able to track changes, showing the validity of the two station names we have defined. 
